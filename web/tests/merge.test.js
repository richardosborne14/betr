...
test('every result the app writes has its own id and the word that was tapped', () => {
  const a = boot();
  a.tap('#go').tap('[data-id]', 0).tap('#lock').tap('#done');
  ...
  assert.deepStrictEqual(saved.done.map((d) => d.id), ['no', 'no']);
...
test('two waiting tests at once have two different ids', () => {
  const a = boot();
  a.tap('#go').tap('[data-id]', 0).tap('#lock');
  a.tap('#m-new').tap('[data-id]', 2).tap('#lock');
  a.tap('#m-new').tap('[data-id]', 4).tap('#lock');
  a.tap('#m-mine');
