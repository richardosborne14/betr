/*
  Which country a time zone is in. Data, not logic — web/lib/where.js does the looking up.

  Generated from the IANA time zone database on a machine, version 2026c: /usr/share/zoneinfo,
  which is in the public domain. `zone.tab` gives one ISO 3166-1 country code per zone; the
  legacy zone names underneath were matched by comparing the compiled zone files byte for byte,
  and the handful that byte-matching cannot separate are named in the generator's ALIASES.

  Why this file exists at all: `Intl.DateTimeFormat().resolvedOptions().timeZone` is the one
  signal about where a person is that costs nothing — no request, no permission prompt, no
  sensor, nothing stored and nothing sent (B17, CLAUDE.md rule 1). The browser already knows
  it. It says "Africa/Lagos" and this file says that means Nigeria.

  It is deliberately not a geo-IP lookup and never becomes one, and the Geolocation API is off
  the table permanently: a private mental-health app asking for your location is the most
  alarming thing it could do.

  Reading it: one line per country, its zones in one string, split at load. Ambiguous names
  (EST, CET, GMT, UTC) are absent on purpose — an unknown country is a valid answer here, and
  a guessed one is not.

  To refresh it after a tzdb release, re-run the generator in the B17 task file. Country
  borders and zone names change; helpline numbers change faster, and those live in
  content/helplines.js with the date a person last checked each one.
*/
var BETR_ZONES = {
  AD: "Europe/Andorra",
  AE: "Asia/Dubai",
...
if (typeof module === 'object' && module.exports) module.exports = BETR_ZONES;
else (self.Betr = self.Betr || {}).zones = BETR_ZONES;
