...
Eleven screens, **76 tests**, no dependencies, no build step, no requests after the page loads.

**2026-09-02:** "fear" became **worry** everywhere. The re-rate moves a **1–10 ladder** instead
of storing a fixed number. **Your worries** was added: one card per belief, its ladder, what was
written each time, and *Test this again*.

**2026-09-03, morning:** the wordmark is **BETR**, all caps, and no label a person taps is
all-lowercase. Both guarded by tests.
...
- **A test you locked in waits for you.** `S.open`, no cap, nothing counts them. Leaving the
  loop parks what is in hand — it is in `go()`, so it holds for every way out, not just *New
  worry*. Front screen shows one line; Your worries carries them on their cards with **Done it**
...
1. **B1: write the twelve items fresh** — `docs/tasks/B1-the-stock-list.md`. The shape, the file
   and the tests are done; the words are the prototype's and are a placeholder. Then Misha, then
   one paid CBT reviewer. This is the product; everything else is plumbing.
...
> write the twelve worries fresh. Every phrase written from nothing — CCI, Getselfhelp,
> Therapist Aid, Psychology Tools and the Beck Institute all forbid reuse, and `content.test.js`
> will not catch a borrowed sentence. Keep the six parts, the lanes and the ids exactly as they
> are: stored results point at the ids. The first three get most of the taps, so they must be
> the easiest to succeed at. No test may touch the habit itself; the guard enforces it and the
> guard is not the point — the writing is. Then `node --test`, update the task file, and rewrite
> this file.

**Not for a coding session, and blocking release:**

- **Walk J1, J2 and J3 on a phone.** `docs/journeys.md`. **J3 is new and matters most right
  now**: the menu on a short screen, and a test that survives *New worry* and a restart.
- **Misha signs off the Help link list** (`web/content/places.js`, `signedOff: false`) and the
  six surface-problem labels.
...
| Drive it for real | headless Chrome + CDP over plain `fetch`/`WebSocket` in Node 22, no dependencies. **Set the viewport with `Emulation.setDeviceMetricsOverride`, never `--window-size`** |
...
- **`node --test web/tests/` does not work on Node 22.** Use `node --test` from the repo root.
  `web/tests/harness.js` is the fake DOM, shared by `loop.test.js` and `menu.test.js`; it is not
  a test file and node does not pick it up.
...
- **Storage is at version 2**, and `S.open` was added without a bump: a state saved before B8
  simply has no `open`, and `normalise` gives it an empty one. Don't delete the v1 `rate`
  migration.
...
- **Every worksheet phrase must be fresh.** BETR links to CCI and Getselfhelp; it takes no words
  from them, and that does not change because we now link to them.
...
became 76. The fake DOM moved to `web/tests/harness.js`. J3 written; J1 and J2 updated for the
