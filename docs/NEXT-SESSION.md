...
**2026-09-03, later the same day: B19 is proposed and waiting on the founder.** The founder
watched two test users. Both stalled on the pick list, because a worry's `belief` — the
"If I ___, then ___" that is the only self-explaining part of an item — is not drawn until
the re-rate, four screens after they choose. `docs/tasks/B19-doors-first.md` proposes making
the doors the way in, putting the belief sentence under the label on the pick button, and
going to **nineteen worries, four or five per door** (the twelve keep their ids, so B18's
`why.js` entries stay valid; seven are new and all seven pass `guards.checkTest`). **Two
decisions are the founder's** — shape A or B for the front screen, and how plainly door one
names drink and drugs against frozen sentence 4 — and Misha now signs off six rewritten doors
rather than six existing ones. Nothing in `web/` has changed.

**Also new: `docs/COPY.md`** — every word BETR can show, generated by `node tools/copy-sheet.js`
from `strings-en.js`, `worries.js`, `whats-going-on.js`, `why.js` and `places.js`. Regenerate
it after any content edit; never hand-edit it. The founder asked for one place to read and
mark up the copy, and a hand-written one would be wrong within a week.

**Otherwise there isn't a coding one.** B10 must not start until passkeys-in-a-Capacitor-webview is
