...
**91 candidate lines.** Read them the way you would read a colleague's first draft. §5 is
an audit of them against three failure shapes measured the same day, and it recommends cutting
...
| 5 | the whole day will run behind | a practical consequence |
...
| 4 | Don't check afterwards how it landed. | **new** |
| 5 | Don't warn anybody that it's coming. | **new** |
| 6 | Don't do it at half strength. | **new** |

**One caution we can't settle ourselves.** Line 5 is a good drop for a social test and wrong
for somebody who genuinely ought to tell a person what they're about to do. Generic lines can't
tell those apart. If that risk isn't acceptable, cut it — the set works at five.
...
it landed"* from the general set. It leaves five generic drops, which is enough, and it means
no unbounded line in BETR asks anybody to stop checking. Keep the four bounded ones.
...
**One mechanical note for whoever moves these across.** This file uses straight apostrophes
because it is for reading; `starts.js` uses curly ones throughout. They need converting on the
way in, or the app will show two different apostrophes on one screen.

**Already checked, so nobody repeats it:** all 91 lines were run against the `HABIT`, `BODY`
and `HARM` word lists in `web/lib/guards.js` — no hits — and against the shape rules
`content.test.js` applies (capital, full stop, no doubled spaces, no duplicates). That says
nothing about whether they are clinically right, which is the whole reason this file needs you.
